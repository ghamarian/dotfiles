...
export ruleSearchTerm="*$2*"

export files=`ssh root@augurs "(docker exec -i $dockerID grep --include="*.java" -irlP '^(?!.*import).${ruleSearchTerm}' /usr/share/tomcat7/work/Catalina/localhost/prweb/PRGenJava)"` 
export nrFiles=`echo $files | awk '{print NF}'`
export files=`echo $files| xargs`
export files=$(echo ${files} | sed 's/\$/\\\$/g')

echo "Getting $nrFiles rules $ruleSearchTerm from $dockerName($dockerID)"

[[ $nrFiles -gt 30 ]] && echo too many files, make your search more refined! && exit 1
