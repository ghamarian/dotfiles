...
ssh $serverName "docker exec -i $dockerID /tomcat/cassandra/bin/cqlsh -u dnode_ext -p dnode_ext $IP -e 'select fk from adm.adm_scoringmodel'" | tail -n +4  | jq '.modelPartition.partition'
